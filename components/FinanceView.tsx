...
const FinanceView: React.FC<FinanceViewProps> = ({ 
    ...
    const handleTransfer = () => {
        ...
        onAddTransaction({
            ...
            description: `Virement vers ${comptes.find(c => c.id === destId)?.nom}${note ? `. Motif: ${note}` : ''}`, categorie: 'TRANSFERT'
        });
        onAddTransaction({
            id: `TR_V_IN_${Date.now()}`, date, type: 'VIREMENT_ENTRANT', montant, compteId: destId, 
            description: `Virement depuis ${source?.nom}${note ? `. Motif: ${note}` : ''}`, categorie: 'TRANSFERT'
        });
                                ...
                                return (
                                    <div key={compte.id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all group flex flex-col justify-between h-[160px] relative">
                                        ...
                                        <div className="flex justify-between items-start">
                                            ...
                                            {boutique && <span className="text-[8px] bg-brand-900 text-white border border-brand-800 px-2 py-1 rounded-lg uppercase font-black shadow-sm">{boutique.nom}</span>}
                                        </div>
