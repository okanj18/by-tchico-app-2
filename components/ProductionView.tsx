...
const ProductionView: React.FC<ProductionViewProps> = ({ 
    ...
    const handleCreateCustomOrder = () => {
        ...
        const order: Commande = {
            ...
            elements: newOrderItems.map(i => ({nom: i.nom, quantite: i.qte})), taches: []
        };
    ...
    const generateInvoice = (order: Commande) => {
        ...
        const html = `<html><head><title>Facture</title><style>body{font-family:sans-serif;padding:40px;} .h{display:flex;justify-content:space-between;border-bottom:2px solid #68321f;padding-bottom:20px;margin-bottom:30px;} table{width:100%;border-collapse:collapse;} th,td{padding:10px;text-align:left;border-bottom:1px solid #eee;}</style></head><body><div class="h"><div><h2>BY TCHICO</h2><p>${COMPANY_CONFIG.address}</p></div><div><h3>FACTURE #${order.id.slice(-6)}</h3><p>Date: ${new Date(order.dateCommande).toLocaleDateString()}</p></div></div><h4>Client: ${order.clientNom}</h4><table><tr><th>Article</th><th>Qte</th></tr>${order.elements?.map(e => `<tr><td>${e.nom}</td><td>${e.quantite}</td></tr>`).join('')}</table><div style="margin-top:30px;text-align:right;"><h3>TOTAL: ${order.prixTotal.toLocaleString()} F</h3><p>Payé: ${order.avance.toLocaleString()} F</p><h2 style="color:red;">RESTE: ${order.reste.toLocaleString()} F</h2></div></body></html>`;
    ...
    const handleTaskStatusChange = (order: Commande, task: TacheProduction, newStatut: 'A_FAIRE' | 'FAIT') => {
        ...
        if (rule) {
            ...
            if (qtyToMove > 0) {
                if (newStatut === 'FAIT') { newRepartition[rule.from] -= qtyToMove; if (newRepartition[rule.from] <= 0) delete newRepartition[rule.from]; newRepartition[rule.to] = (newRepartition[rule.to] || 0) + qtyToMove; } 
                else { newRepartition[rule.to] -= qtyToMove; if (newRepartition[rule.to] <= 0) delete newRepartition[rule.to]; newRepartition[rule.from] = (newRepartition[rule.from] || 0) + qtyToMove; }
            }
        }
    ...
    return (
        <div className="space-y-6 h-[calc(100vh-8rem)] flex flex-col">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => { setNewOrderItems([{nom: '', qte: 1}]); setInitialAccountId(''); setOrderModalOpen(true); }} className="bg-brand-900 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-[10px] font-black uppercase hover:bg-black shadow-lg transition-all active:scale-95"><Plus size={16}/> Nouvelle Commande</button>
                ...
                <div className="flex flex-wrap bg-white border p-1 rounded-xl shadow-sm">
                    {[{id: 'PLANNING', label: 'Agenda', icon: Calendar},{id: 'KANBAN', label: 'Kanban', icon: Columns},{id: 'ORDERS', label: 'Commandes', icon: LayoutList},{id: 'TAILORS', label: 'Artisans', icon: Users},{id: 'PERFORMANCE', label: 'Top Performance', icon: Trophy}].map((mode) => (
                ...
                {viewMode === 'KANBAN' && (
                    <div className="flex gap-4 h-full overflow-x-auto pb-4 custom-scrollbar">
                        {KANBAN_STATUS_ORDER.map((status) => (
                            <div key={status} className="flex-1 min-w-[280px] bg-gray-100/50 rounded-xl flex flex-col border border-gray-200">
                                <div className="p-3 border-b flex justify-between items-center bg-white rounded-t-xl shrink-0"><h3 className="text-xs font-black text-gray-700 uppercase tracking-widest">{status}</h3><span className="bg-brand-900 text-white text-[10px] px-2 py-1 rounded-full font-black">{filteredCommandes.reduce((acc, c) => acc + (c.repartitionStatuts?.[status] || (c.statut === status ? c.quantite : 0)), 0)} pcs</span></div>
                                <div className="flex-1 overflow-y-auto p-2 space-y-3">
                                    {filteredCommandes.filter(c => (c.repartitionStatuts?.[status] || 0) > 0 || (!c.repartitionStatuts && c.statut === status)).map(order => {
                                        ...
                                        return (
                                            <div key={order.id} className="bg-white p-3 rounded-lg shadow-sm border border-gray-200 group">
                                                <div className="flex justify-between items-start mb-1 text-[10px] font-black text-gray-400"><span>#{order.id.slice(-6)}</span><span className="bg-brand-50 text-brand-700 px-1.5 rounded-lg border border-brand-100">QTÉ: {qtyInColumn}</span></div>
                                                <p className="font-black text-gray-800 text-sm mb-1 uppercase">{order.clientNom}</p>
                                                <p className="text-[10px] text-gray-500 italic line-clamp-1 border-l-2 border-brand-100 pl-2 mb-2">{order.description}</p>
                                            </div>
                                        );
                                    })}
                                </div>
            ...
            {/* --- MODAL NOUVELLE COMMANDE (REVISITÉ) --- */}
            {orderModalOpen && (
                <div className="fixed inset-0 bg-brand-900/80 z-[250] flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-10 flex flex-col max-h-[90vh] animate-in zoom-in duration-300 border border-brand-100">
                            ...
                            <div className="space-y-4">
                                ...
                                {newOrderItems.map((item, idx) => (
                                    <div key={idx} className="flex gap-3 items-center animate-in slide-in-from-left duration-200">
                                        <input type="text" placeholder="Nom de l'article (ex: Chemise Bazin)" className="flex-1 p-4 border-2 border-gray-100 rounded-2xl text-sm font-bold bg-gray-50 focus:border-brand-600 outline-none" value={item.nom} onChange={e => { const list = [...newOrderItems]; list[idx].nom = e.target.value; setNewOrderItems(list); }} />
                                        <input type="number" min="1" className="w-20 p-4 border-2 border-gray-100 rounded-2xl text-center font-black bg-gray-50 focus:border-brand-600 outline-none" value={item.qte} onChange={e => { const list = [...newOrderItems]; list[idx].qte = parseInt(e.target.value)||1; setNewOrderItems(list); }} />
                        ...
                        <div className="flex justify-end gap-4 mt-10 shrink-0 border-t pt-6">
                            <button onClick={() => setOrderModalOpen(false)} className="px-8 py-4 text-gray-400 font-black uppercase text-[10px] tracking-[0.2em] hover:text-gray-600">Annuler</button>
            ...
            {/* --- MODAL DÉTAILS & HISTORIQUE FINANCIER --- */}
            {orderDetailView && (
                <div className="fixed inset-0 bg-brand-900/90 z-[300] flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col h-[85vh] animate-in slide-in-from-bottom-8 duration-300">
                        <div className="p-8 border-b bg-gray-50 flex justify-between items-center rounded-t-3xl shrink-0">
                            <div><h3 className="text-2xl font-black text-gray-800 uppercase tracking-tighter">Détails Commande</h3><p className="text-[10px] text-gray-400 font-mono tracking-widest mt-1">REF: #{orderDetailView.id}</p></div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                            ...
                            <div className="grid grid-cols-2 gap-8 border-b pb-8">
                                <div><h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Articles Commandés</h4><div className="space-y-2">{orderDetailView.elements?.map((e, i) => (<div key={i} className="flex justify-between items-center bg-gray-50 p-3 rounded-xl border border-gray-100"><span className="text-sm font-bold text-gray-800">{e.nom}</span><span className="font-black text-brand-600">x{e.quantite}</span></div>))}</div></div>
                                <div><h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-3">Dates Clés</h4><div className="space-y-3"><div className="flex justify-between text-sm"><span className="text-gray-500">Commande :</span><span className="font-bold">{new Date(orderDetailView.dateCommande).toLocaleDateString()}</span></div><div className="flex justify-between text-sm"><span className="text-gray-500">Livraison prévue :</span><span className="font-black text-brand-700 underline decoration-2">{new Date(orderDetailView.dateLivraisonPrevue).toLocaleDateString()}</span></div></div></div>
                            </div>
                            {/* Recap Finance */}
                            <div>
                                <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Historique des Règlements</h4>
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center p-4 bg-brand-900 text-white rounded-2xl shadow-lg mb-6"><span className="text-xs font-black uppercase tracking-widest">Montant Total TTC</span><span className="text-2xl font-black">{orderDetailView.prixTotal.toLocaleString()} F</span></div>
                                    <div className="space-y-2">
                                        {orderDetailView.paiements?.map((p, i) => (
                                            <div key={i} className="flex justify-between items-center bg-green-50 p-4 rounded-2xl border-l-4 border-green-600 animate-in fade-in slide-in-from-right duration-300">
                            ...
                            {/* Recap Production */}
                            <div><h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Artisans impliqués</h4><div className="flex flex-wrap gap-2">{Array.from(new Set(orderDetailView.taches?.map(t => employes.find(e => e.id === t.tailleurId)?.nom))).map((nom, i) => (<span key={i} className="px-4 py-2 bg-gray-100 text-gray-800 rounded-xl text-xs font-black uppercase tracking-tighter border border-gray-200">{nom}</span>))}</div></div>
                        </div>
                        <div className="p-8 border-t bg-gray-50 flex justify-between shrink-0 rounded-b-3xl">
                             <button onClick={() => generateInvoice(orderDetailView)} className="px-6 py-3 bg-gray-800 text-white rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 shadow-lg"><Printer size={16}/> Imprimer Ticket</button>
                             <button onClick={() => setOrderDetailView(null)} className="px-6 py-3 text-gray-500 font-black uppercase text-[10px] tracking-widest">Fermer</button>
                        </div>
                    </div>
                </div>
            )}

            {/* MODAL LIVRAISON (ACTUALISÉ) */}
            {deliveryModal && (
                <div className="fixed inset-0 bg-brand-900/80 z-[350] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-10 text-center border-t-8 border-green-500 animate-in zoom-in duration-200">
                        ...
                        <p className="text-sm text-gray-500 mb-8 font-bold">Confirmer la sortie de <span className="text-gray-900">{deliveryModal.order.clientNom}</span> ?</p>
            ...
            {taskModalOpen && (
                <div className="fixed inset-0 bg-brand-900/70 z-[200] flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-8 animate-in zoom-in duration-300 border border-gray-100">
                        ...
                        <div className="flex justify-end gap-3 mt-10"><button onClick={() => setTaskModalOpen(false)} className="px-6 py-4 text-gray-400 font-black uppercase text-[10px] tracking-widest">Annuler</button><button disabled={!newTaskData.tailleurId || !newTaskData.orderId} onClick={() => {
                        ...
                        }} className="px-10 py-4 bg-brand-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl disabled:opacity-30 active:scale-95">Confirmer</button></div>
                    </div>
                </div>
            )}
        </div>
