...
const ProductionView: React.FC<ProductionViewProps> = ({ 
    ...
    const handleAddConsommation = () => {
        ...
        if (!tempCons.articleId || isNaN(qty) || qty <= 0) {
            alert("Veuillez choisir un article et saisir une quantité valide.");
            return;
        }
        ...
        if (stock < qty) { 
            alert(`Stock insuffisant à l'Atelier (${stock} ${art.unite} dispo).`); 
            return; 
        }
        
        setNewOrderData({ 
            ...newOrderData, 
            consommations: [...(newOrderData.consommations || []), { ...tempCons, quantite: qty, id: `CONS_${Date.now()}` }] 
        });
    ...
    return (
        <div className="space-y-6">
            {/* HEADER AVEC BOUTONS D'ACTION RESTAURÉS */}
                ...
                <div className="bg-white rounded-[2rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col min-h-[600px] animate-in fade-in">
                    <div className="p-6 border-b flex justify-between items-center bg-gray-50/50">
                        <div className="flex gap-1">
                            ...
                            <h3 className="text-xl font-black text-gray-800 uppercase tracking-widest ml-4">{agendaDate.toLocaleDateString('fr-FR',{month:'long',year:'numeric'})}</h3>
                        </div>
                    </div>
            ...
            {/* KANBAN NOMINATIF AVEC ASSIGNATION RAPIDE */}
            {activeNav === 'KANBAN' && (
                        ...
                        return (
                            <div key={status} className="flex flex-col min-w-[320px] max-w-[320px] h-full">
                                <div className="flex justify-between items-center mb-6 px-2">
                                    ...
                                    <span className="bg-brand-50 text-brand-700 px-2 py-0.5 rounded-full text-[9px] font-black border border-brand-100">{items.length} flux</span>
                                </div>
                                            ...
                                            <div className="space-y-3">
                                                <div className="flex justify-between items-start">
                                                    <div onClick={()=>{setSelectedOrder(item.order);setDetailModalOpen(true);}} className="cursor-pointer">
                                                        <h5 className="font-black text-gray-900 uppercase text-xs mb-1">{item.order.clientNom}</h5>
                                                        <p className="text-[8px] text-gray-400 font-bold uppercase tracking-widest">Réf: #{item.order.id.slice(-6)}</p>
                                                    </div>
                                                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg ${item.isAssigned ? 'bg-brand-900 text-white' : 'bg-gray-100 text-gray-400'}`}>{item.qty}/{item.total} {item.element}</span>
                                                </div>
                                                
                                                {item.isAssigned ? (
                                                    <div className="flex items-center gap-2 px-3 py-2 bg-white rounded-xl border border-brand-100 shadow-inner">
                                                        <div className="w-6 h-6 bg-brand-600 rounded-lg flex items-center justify-center text-white"><UserCheck size={12}/></div>
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-[8px] font-black text-brand-800 uppercase truncate">{item.artisanName || 'En cours'}</p>
                                                            <p className="text-[7px] font-bold text-brand-400 uppercase">Artisan Assigné</p>
                                                        </div>
                                                    </div>
                                                ) : (
                                                    status !== StatutCommande.PRET && userRole !== RoleEmploye.VENDEUR && (
                                                        <button 
                                                            onClick={() => {
                                                                ...
                                                                setMultiTasks([{elementNom: item.element, action: item.action as ActionProduction, quantite: item.qty}]);
                                                                setTaskModalOpen(true);
                                                            }}
                                                            className="w-full py-2.5 bg-gray-50 hover:bg-brand-900 hover:text-white rounded-xl border border-dashed border-gray-200 text-[8px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
                                                        >
                                                            <UserPlus size={12} /> Assigner cette pièce
                                                        </button>
                                                    )
                                                )}
                                                
                                                {status === StatutCommande.PRET && (
                                                    <button onClick={()=>{setSelectedOrder(item.order); setDeliveryModalOpen(true);}} className="w-full py-2.5 bg-green-50 hover:bg-green-600 hover:text-white rounded-xl text-[8px] font-black uppercase tracking-widest text-green-700 transition-all flex items-center justify-center gap-2"><Truck size={12}/> Prêt pour livraison</button>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                            ...
                            return (
                                <div key={artisan.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 hover:bg-white hover:shadow-xl transition-all">
                                    <div className="flex items-center gap-6">
                                        <span className="text-2xl font-black text-gray-300 w-8 text-center">#{i+1}</span>
                                    ...
                                    <div className="flex items-center gap-10">
                                        <div className="text-right"><p className="text-[10px] font-black text-gray-400 uppercase mb-0.5">Tâches Finies</p><p className="text-xl font-black text-gray-800">{finishedTasks}</p></div>
                                        <div className="w-40 h-2 bg-gray-200 rounded-full overflow-hidden shadow-inner"><div className="h-full bg-brand-900 transition-all duration-1000" style={{ width: `${Math.min(100, (finishedTasks/10)*100)}%` }} /></div>
                                    </div>
                                </div>
                            );
                        })}
            ...
            {/* MODAL FICHE PRODUCTION RESTAURÉ AVEC TRACABILITÉ ARTISANS */}
                ...
                <div className="fixed inset-0 bg-brand-900/90 z-[600] flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col max-h-[95vh] animate-in zoom-in border border-brand-100">
                        <div className="p-8 border-b flex justify-between items-center bg-white shrink-0">
                            ...
                            <div className="flex gap-2">
                                <button onClick={()=>{setDetailModalOpen(false); setNewOrderData(selectedOrder); setIsEditingOrder(true); setOrderModalOpen(true);}} className="p-3 bg-gray-100 text-gray-600 rounded-xl hover:bg-brand-900 hover:text-white transition-colors"><Edit2 size={24}/></button>
                                <button onClick={() => setDetailModalOpen(false)} className="p-3 bg-gray-50 text-gray-400 rounded-xl hover:bg-gray-100"><X size={24}/></button>
                            </div>
                        </div>
                        <div className="p-10 overflow-y-auto flex-1 bg-gray-50/30 space-y-8 custom-scrollbar">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-1 gap-4">
                                    {selectedOrder.elements?.map(el => {
                                        ...
                                        return (
                                            <div key={el.id} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden group">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div>
                                                        <span className="text-lg font-black uppercase text-gray-800">{el.nom}</span>
                                                        <p className="text-[10px] text-gray-400 font-bold uppercase">Quantité Totale: {el.quantiteTotal}</p>
                                                    </div>
                                                    <span className="text-3xl font-black text-brand-900 opacity-20">x{el.quantiteTotal}</span>
                                                </div>
                                                
                                                {/* Barre de progression Coupe */}
                                                <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden mb-6"><div className="h-full bg-brand-900 transition-all duration-1000" style={{ width: `${progress}%` }} /></div>
                                                
                                                {/* Détails par étape / Artisan */}
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                                    {['COUPE', 'COUTURE', 'FINITION'].map((act) => {
                                                        const tasksForAction = elementTasks.filter(t => t.action === act);
                                                        return (
                                                            <div key={act} className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100">
                                                                <p className="text-[8px] font-black text-gray-400 uppercase mb-2 tracking-widest">{act}</p>
                                                                {tasksForAction.length > 0 ? (
                                                                    <div className="space-y-2">
                                                                        {tasksForAction.map(t => {
                                                                            const art = employes.find(e => e.id === t.tailleurId);
                                                                            return (
                                                                                <div key={t.id} className="flex items-center justify-between">
                                                                                    <div className="flex items-center gap-2">
                                                                                        <div className={`w-2 h-2 rounded-full ${t.statut === 'FAIT' ? 'bg-green-500' : 'bg-orange-400 animate-pulse'}`} />
                                                                                        <span className="text-[10px] font-black text-gray-700 uppercase">{art?.nom || 'Inconnu'}</span>
                                                                                    </div>
                                                                                    <span className="text-[9px] font-bold text-gray-400">({t.quantite})</span>
                                                                                </div>
                                                                            );
                                                                        })}
                                                                    </div>
                                                                ) : (
                                                                    <p className="text-[9px] text-gray-300 italic font-bold uppercase">En attente</p>
                                                                )}
                                                            </div>
                                                        );
                                                    })}
                            ...
                            <div className="bg-white rounded-[2rem] p-8 border border-brand-100 shadow-2xl">
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                                    <div><p className="text-[10px] font-black text-gray-400 uppercase mb-1">Montant Total</p><p className="text-3xl font-black text-gray-800">{selectedOrder.prixTotal.toLocaleString()} F</p></div>
                                    <div><p className="text-[10px] font-black text-emerald-500 uppercase mb-1">Total Payé</p><p className="text-3xl font-black text-emerald-600">{(selectedOrder.prixTotal - selectedOrder.reste).toLocaleString()} F</p></div>
                                    <div className="bg-red-50 p-4 rounded-2xl border border-red-100"><p className="text-[10px] font-black text-red-400 uppercase mb-1">Reste Final</p><p className="text-3xl font-black text-red-600">{selectedOrder.reste.toLocaleString()} F</p></div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 bg-brand-900/80 z-[700] flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in border border-brand-100">
                         <div className="p-8 bg-white border-b flex justify-between items-center shrink-0">
                            <h3 className="text-xl font-black text-gray-800 uppercase tracking-tighter flex items-center gap-3"><Truck className="text-brand-600"/> Bon de Livraison</h3>
                            <button onClick={()=>setDeliveryModalOpen(false)}><X size={28}/></button>
                         </div>
                         <div className="p-8 space-y-6">
                            <div className="bg-gray-50 p-6 rounded-3xl border text-center">
                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Reste à payer avant remise</p>
                                <p className={`text-3xl font-black ${selectedOrder.reste > 0 ? 'text-red-500' : 'text-green-600'}`}>{selectedOrder.reste.toLocaleString()} F</p>
                            </div>
                            <div className="space-y-4">
            ...
            {/* MODAL ENCAISSEMENT RELIQUAT ACTIF */}
            ...
            {/* MODAL NOUVELLE COMMANDE RESTAURÉ AVEC STOCK */}
            {orderModalOpen && (
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[95vh] border border-brand-100 overflow-hidden animate-in zoom-in">
                         ...
                         <div className="p-8 space-y-6 overflow-y-auto flex-1 custom-scrollbar bg-gray-50/30">
                            ...
                            <div className="bg-white p-6 rounded-[2rem] border border-brand-100 space-y-4 shadow-inner">
                                <h4 className="text-[10px] font-black text-brand-700 uppercase tracking-widest flex items-center gap-2"><List size={14}/> Composition (ex: 3 Vestes, 3 Pantalons)</h4>
            ...
            {/* MODAL ASSIGNATION TÂCHES RESTAURÉ AVEC BLOCAGE SÉCURITÉ */}
            {taskModalOpen && (
                <div className="fixed inset-0 bg-brand-900/80 z-[800] flex items-center justify-center p-4 backdrop-blur-md">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-xl flex flex-col max-h-[90vh] animate-in zoom-in duration-200 border border-brand-100 overflow-hidden">
                        ...
                        <div className="p-8 space-y-6 overflow-y-auto flex-1 bg-gray-50/30 custom-scrollbar">
                            ...
                            <div className="space-y-4">
                                ...
                                {multiTasks.map((t,idx)=>{
                                    ...
                                    return (
                                        <div key={idx} className={`bg-white p-6 rounded-2xl border shadow-sm flex flex-col md:flex-row gap-4 items-end transition-all ${isError ? 'border-red-500 bg-red-50/20' : 'border-gray-100'}`}>
                                            <div className="flex-1 w-full"><label className="block text-[8px] font-black text-gray-400 uppercase mb-1">Pièce</label><select className="w-full p-2.5 border border-gray-100 rounded-xl font-bold text-xs" value={t.elementNom} onChange={e=>{const u=[...multiTasks];u[idx].elementNom=e.target.value;setMultiTasks(u);}}><option value="">-- Choisir --</option>{cmd?.elements?.map(el=><option key={el.id} value={el.nom}>{el.nom}</option>)}</select></div>
                                            <div className="w-full md:w-32"><label className="block text-[8px] font-black text-gray-400 uppercase mb-1">Action</label><select className="w-full p-2.5 border border-gray-100 rounded-xl font-bold text-xs" value={t.action} onChange={e=>{const u=[...multiTasks];u[idx].action=e.target.value as ActionProduction;setMultiTasks(u);}}><option value="COUPE">COUPE</option><option value="COUTURE">COUTURE</option><option value="FINITION">FINITION</option></select></div>
                                            <div className="w-full md:w-24">
                                                <label className={`block text-[8px] font-black uppercase mb-1 ${isError ? 'text-red-600' : 'text-gray-400'}`}>Qté (Max: {maxAllowed})</label>
                                                <input type="number" className={`w-full p-2.5 border rounded-xl font-black text-center text-xs outline-none transition-all ${isError ? 'border-red-600 bg-white ring-4 ring-red-100' : 'border-gray-100'}`} value={t.quantite} onChange={e=>{const u=[...multiTasks];u[idx].quantite=parseInt(e.target.value)||0;setMultiTasks(u);}}/>
                                            </div>
                                            <button onClick={()=>setMultiTasks(multiTasks.filter((_,i)=>i!==idx))} className="p-2.5 text-red-300 hover:text-red-600"><Trash2 size={18}/></button>
                                        </div>
                                    );
                                })}
                            </div>
                        ...
                        <div className="p-8 bg-gray-50 border-t flex flex-col gap-4">
                            {isTaskAssignmentInvalid && taskBaseData.commandeId && taskBaseData.tailleurId && multiTasks.length > 0 && (
                                <p className="text-[10px] font-black text-red-600 uppercase text-center flex items-center justify-center gap-2"><AlertTriangle size={14}/> Certaines quantités dépassent le quota disponible</p>
                            )}
                            <div className="flex justify-end gap-3">
                                ...
                                <button onClick={()=>{
                                        const cmd=commandes.find(c=>c.id===taskBaseData.commandeId);
                                        if(!cmd) return;
                                        multiTasks.forEach(mt=>{if(mt.elementNom && mt.quantite!>0) onAddTask(cmd.id,{id:`T_${Date.now()}_${Math.random()}`,commandeId:cmd.id,elementNom:mt.elementNom!,action:mt.action as ActionProduction,quantite:mt.quantite!,date:taskBaseData.date,tailleurId:taskBaseData.tailleurId,statut:'A_FAIRE'});});
                                        setTaskModalOpen(false); setMultiTasks([]);
                                    }} disabled={isTaskAssignmentInvalid} className="px-12 py-4 bg-brand-900 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 disabled:opacity-30 disabled:grayscale disabled:cursor-not-allowed transition-all">Assigner Travail</button>
                            </div>
                        </div>
                    </div>
                </div>
